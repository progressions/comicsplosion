...
//      Corey Donohoe - http://www.atmos.org
...
var calendarFormatTimeString = '';
var calendarIfFormatTime = '';
...
function getDateObj(yyyy, mm, dd, HH, MM) {
    var obj = new Date();
    ...
    obj.setMonth(mm);
    obj.setDate(dd);
    if(HH) obj.setHours(HH);
    if(MM) obj.setMinutes(MM);
...
var dateParsePatterns = [
    ...
    // right now parsing time is only supported with (the default) ISO
    // date format. If you want to make it work with other formats,
    // you'd need to create duplicate entries in the dateParsePatterns
    // array and add time support to them like this one

    // yyyy-mm-dd HH:MM (ISO style) with time
    {   re: /(\d{4})-(\d{1,2})-(\d{1,2}) (\d{1,2}):(\d{1,2})/,
	    ...
	    var MM = parseInt(bits[5], 10);

            if ( DateInRange( yyyy, mm, dd ) )
		return getDateObj(yyyy, mm, dd, HH, MM);
        }
    },
...
function magicDate(id, onlyOnSubmit) {
	...
	if(calendar.params.showsTime) { 
	    input.value = input.value + ' ' + hour + ':' + minute;
	}
...
function setConfigDateType(type) {
  switch (configDateType) {
      ...
      case 'us':
          ...
          calendarIfFormatTime = '%m/%d/%Y %H:%M';
          calendarFormatTimeString = 'mm/dd/yyyy hh:mm';
          break;
      case 'mm.dd.yyyy':
      case 'de':
          calendarIfFormat = '%m.%d.%Y';
      ...
      case 'dd/mm/yyyy':
          ...
          calendarIfFormatTime = '%d/%m/%Y %H:%M';
          calendarFormatTimeString = 'dd/mm/yyyy hh:mm';
          break;
      case 'dd-mm-yyyy':
          calendarIfFormat = '%d-%m-%Y';
          calendarFormatString = 'dd-mm-yyyy';
      ...
      default:
          ...
          calendarIfFormatTime = '%Y-%m-%d %H:%M';
          calendarFormatTimeString = 'yyyy-mm-dd hh:mm';
          break;
  }
}